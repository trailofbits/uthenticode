...
TEST(read_certs, handles_nullptr) {
  auto certs = smolverify::read_certs(nullptr);
...
TEST_F(NoAuthTest, get_checksums) {
  auto checksums = smolverify::get_checksums(pe);
...
TEST_F(Auth32Test, get_checksums) {
  ...
  EXPECT_EQ(std::get<smolverify::checksum_kind>(checksums[0]), smolverify::checksum_kind::SHA1);
}
...
TEST_F(NoAuthTest, calculate_checksum) {
  auto unk = smolverify::calculate_checksum(pe, smolverify::checksum_kind::UNKNOWN);
  EXPECT_TRUE(unk.empty());

  auto md5 = smolverify::calculate_checksum(pe, smolverify::checksum_kind::MD5);
  EXPECT_EQ(md5, "6f7ac8c1754fad04fba2a552a122e7");

  auto sha1 = smolverify::calculate_checksum(pe, smolverify::checksum_kind::SHA1);
  EXPECT_EQ(sha1, "4ba4c91418e28cb63b97cfde1cff95a91139");

  auto sha256 = smolverify::calculate_checksum(pe, smolverify::checksum_kind::SHA256);
...
TEST(verify, handles_nullptr) {
  EXPECT_FALSE(smolverify::verify(nullptr));
}

TEST_F(NoAuthTest, verify) {
  EXPECT_FALSE(smolverify::verify(pe));
}

TEST_F(Auth32Test, verify) {
  EXPECT_TRUE(smolverify::verify(pe));
}
